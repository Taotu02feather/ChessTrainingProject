...
class ChessConfig:
    ...
    """残差块的数量。AlphaZero 原始使用 19 或 39，增强训练建议 12~19。"""

    num_filters: int = 192
    """卷积层的滤波器数量。AlphaZero 原始使用 256，增强训练建议 128~256。"""
    ...
    """每步棋的 MCTS 搜索模拟次数。越多越强但越慢。训练建议 100~400。"""

    mcts_c_puct: float = 1.5
    """PUCT 公式中的探索常数 c_puct。控制探索与利用的平衡。增大可增加探索。"""
    ...
    """每轮训练的自我对弈局数。设为 20 可快速验证每轮效果，频繁检查点。"""

    max_moves_per_game: int = 300
    """每局棋的最大步数。设为 300 让棋局自然走到将杀或棋规和棋（50/75步规则），减少人为截断。"""

    resign_threshold: float = -0.99
    """自动认输的估值阈值。设为 -0.99 减少误判早期认输。"""
    ...
    max_training_iterations: int = 200
    """最大训练迭代轮数（每轮包含若干自对弈局数 + 一次训练）。"""

    checkpoint_frequency: int = 1
    """每隔多少轮保存一次模型检查点。设为 1 则每轮都保存。"""
